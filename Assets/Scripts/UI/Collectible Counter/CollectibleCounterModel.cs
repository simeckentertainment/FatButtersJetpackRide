...
public class CollectibleCounterModel : Model
{
    ...
    private void Start()
    {
        ...
        {
            player.UI.ShowInfoText("Collect!\nCollect everything!", collectibleArrowTransform, true);
        }
    }
    ...
    private void CollectiblesCompleted()
    {
        if (showCollectionInfoMessages)
        {
            player.UI.ShowInfoText("Success!\nGet to the finish!", corgiSenseArrowTransform, true);
        }
